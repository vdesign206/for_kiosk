...
const applyMaskToChars = (container) => {
  ...
  chars.forEach(c => {
    // .char 내부를 <span class="char-inner">로 감싸고, .char에 마스크 적용
...
const buildGalleryLayout = () => {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    for (let t = 0; t < maxTriesPerItem; t++) {
      ...
      let ok = true;
      for (const b of boxes) if (overlaps(candidate, b)) { ok = false; break; }
      if (!ok) continue;
...
const showThumbOverlay = () => {
  ...
  title.textContent = "Welcome to Seoul Jungsu Visual Design";
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    card.addEventListener("click", (e) => {
      e.stopPropagation();
      if (isTransitioning) return; // <-- 오타 주의! 아래에서 수정됨
      hideThumbOverlay(); 
